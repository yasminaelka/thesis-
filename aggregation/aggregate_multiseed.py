"""
Aggregate fairness and faithfulness over five seeds.

This script reads the per-seed fairness and faithfulness files, prints summary
tables, and saves LaTeX plus JSON outputs.
"""
...
def mean_std(vals):
    # Mean and std, ignoring missing values
...
# Store fairness values before summarising
...
# Read fairness files for all seeds
for s in SEEDS:
    ...
    for ax in AXES:
        ...
        # Keep only reliable articles
        rel  = {name: a for name, a in arts.items() if a['reliable']}

        # Collect article-level values
        ...
        # Count CIs crossing zero
        n_cross = int(sum(1 for c in cross if c))
        n_total = len(cross)

        # Average reliable articles for this seed
...
# Store faithfulness scores
faith = {k: {'suff_base': [], 'suff_scm': [], 'comp_base': [], 'comp_scm': []} for k in KS}

per_seed_faith = {}

# Read faithfulness files for all seeds
for s in SEEDS:
