...
ap.add_argument('--ethnicity_source', default='keyword',
                choices=['keyword', 'ner'],
                help="how to define the ethnicity protected group. "
                     "'keyword' = whole-word regex (original). "
                     "'ner' = NER NORP minority mask from ner_groups.json.")
args = ap.parse_args()
...
if args.ethnicity_source == 'ner':
    OUT_PATH = os.path.join(OUTPUT_DIR, 'fairness_ci_eod_ner.json')
else:
...
if args.ethnicity_source == 'ner':
    ...
    assert os.path.exists(ner_path), f'missing {ner_path} (run ner_groups.py first)'
    ner_data = json.load(open(ner_path))
    saved_kw = np.array(ner_data['keyword_ethnicity_mask'], dtype=bool)
    live_kw  = build_group_mask(test_texts, ETHNICITY_KEYWORDS)
    assert len(saved_kw) == len(live_kw) == len(test_y), \
        f'length mismatch: json={len(saved_kw)} live={len(live_kw)} test={len(test_y)}'
    n_disagree = int((saved_kw != live_kw).sum())
    assert n_disagree == 0, (
        f'NER mask alignment check FAILED: keyword masks differ on '
        f'{n_disagree} docs. Document order is out of sync with ner_groups.json. '
        f'Re-run ner_groups.py with the current dataset version.')
    NER_ETH_MASK = np.array(ner_data['ethnicity_norp_minority_mask'], dtype=bool)
    print(f'NER ethnicity mask loaded and alignment verified '
          f'(protected={int(NER_ETH_MASK.sum())}).')
...
for axis, kw in AXES.items():
    if axis == 'ethnicity' and NER_ETH_MASK is not None:
        prot = NER_ETH_MASK
    else:
...
meta = {
    ...
    'ethnicity_source': args.ethnicity_source,
    'n_bootstrap': N_BOOT,
